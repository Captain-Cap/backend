...
loger = logging.getLogger('storage')
...
class BalloonsStorage:
    ...
    def add(self, balloon):
        ...
        if self.storage.get(uid):
            loger.error(f"trying to add an existing id {uid}")
    ...
    def delete(self, uid):
        if not self.storage.get(uid):
            loger.error(f"trying to delete a non-existent id {uid}")
    ...
    def update(self, balloon):
        uid = balloon['id']
        if not self.storage.get(uid):
            loger.error(f"trying to change a non-existent id {uid}")
            raise NotFoundError(self.name, f"reason: balloon id {uid} not found")
        self.storage[uid] = balloon
    ...
    def get_balloon_by_id(self, uid):
        if not self.storage.get(uid):
            loger.error(f"calling a non-existent id {uid}")
            raise NotFoundError(self.name, f"reason: balloon id {uid} not found")
